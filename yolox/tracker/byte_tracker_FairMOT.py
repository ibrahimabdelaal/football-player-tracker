...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, temp_feat,classes, buffer_size=30):  # todo: ReID. add inputs of 'temp_feat', 'buffer_size'
...
class BYTETracker(object):
    ...
    def remove_contained_boxes(self,boxes):
        ...
        contained = np.zeros(num_boxes, dtype=bool)
        ...
        filtered_indices = np.where(~contained)[0]
        #print(len(filtered_indices))
        #filtered_boxes = boxes[filtered_indices, :]
        # scores=scores[filtered_indices]
        # classes=classes[filtered_indices]
        # id_feature=id_feature[filtered_indices]
    ...
    def update(self, output_results, img_info, img_size, id_feature):  # TODO: ReID. add 'id_feature'
        ...
        if output_results.shape[1] == 5:
            ...
            b = output_results[:, :4]  # x1y1x2y2, e.g. tlbr
            classes=output_results[:, 6]
            filtered =self.remove_contained_boxes(b)  # x1y1x2y2
            bboxes,scores,classes,id_feature=b[filtered, :],scores[filtered],classes[filtered],id_feature[filtered]
        if len(scores[classes==0])>1:
            max=np.max(scores[classes==0])
            ind=classes!=0
            final_ind=np.logical_and(ind,classes!=0)
            bboxes=bboxes[ind]
            scores=scores[ind]
            classes=classes[ind]
        ...
        if len(dets) > 0:
            ...
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f,c ,30) for
                          # detections to STracks, list [OT_0_(0-0), ...]
                          (tlbr, s,f ,c) in
                          zip(dets, scores_keep, id_feature_keep,classes_keep)]  # class STrack in yolox/tracker/byte_tracker.py
        else:
            detections = []
        ...
        if len(dets_second) > 0:  # TODO: 'f, 30' for ReID
            '''Detections'''
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, f,c, 30) for  # detections_second  --> STrack
                                 (tlbr, s, f,c) in zip(dets_second, scores_second, id_feature_second,classes_second)]
        else:
            detections_second = []
